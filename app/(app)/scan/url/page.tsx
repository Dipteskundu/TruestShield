    ...
    <div className="space-y-6">
      <div className="space-y-2 animate-fade-in-up">
        ...
        <p className="text-muted-foreground">
          Verify links before you click — domain age, blacklists, redirect chains
        </p>
      </div>
      ...
      <div className="flex items-start gap-3 rounded-xl border border-border/50 bg-primary/5 p-4">
        ...
        <p className="leading-relaxed text-sm">{TYPE_GUIDANCE.url}</p>
      </div>
      ...
      {scansData.length > 0 && !result && !loading && (
        <Card>
          <CardContent className="p-0">
            <details className="group">
              ...
              <div className="border-t border-border/50">
                {scansData.map((scan: { id: string; verdict: string; createdAt: string }) => (
                  <button
                    key={scan.id}
      ...
      {result && (
        <Card className="overflow-hidden">
          ...
          <CardContent className="p-6 space-y-6">
            ...
            {result.metadata && <UrlMetadata metadata={result.metadata} verdict={result.verdict} />}
            <div>
